...
namespace CaronteFX
{
  ...
  {      
    ...
    private void GenerateSphereMeshes()
    {
      foreach( var tuple in listClothBodiesGORadius_ )
      {
        ...
        if (crBody != null)
        {
          ...
          bool isBakedColliderMesh = crBody.GetColliderMesh(ref colliderMesh, out colliderMatrixModelToWorld );
    ...
    private void DrawTimers(GUIStyle styleStats)
    {
      ...
      string tRTString = string.Format("{0:D3}h:{1:D2}m:{2:D2}s:{3:D3}ms",
                                         tRT.Hours + (tRT.Days * 24), 
                                         tRT.Minutes, 
                                         tRT.Seconds, 
                                         tRT.Milliseconds);
      EditorGUILayout.LabelField("Real time: ", tRTString, styleStats);

      string tSimString = string.Format("{0:D3}h:{1:D2}m:{2:D2}s:{3:D3}ms", 
                                         tSim.Hours + (tSim.Days * 24), 
                                         tSim.Minutes, 
                                         tSim.Seconds, 
                                         tSim.Milliseconds);
    ...
    private void RenderContactEvent(ContactEventInfo ceInfo, float size)
    {
      ...
      Handles.SphereHandleCap( 0, ceInfo.position_, Quaternion.identity, size, EventType.Repaint );
#else
