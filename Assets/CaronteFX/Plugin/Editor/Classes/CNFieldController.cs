...
namespace CaronteFX
{
  public class CNFieldController : IListController 
  {
    ...
    private bool isBodyField_ = false;
    public bool IsBodyField
    {
      get { return isBodyField_; }
      set { isBodyField_ = value; }
    }
    ...
    //-----------------------------------------------------------------------------------
    public CNFieldController( CommandNode node, CNField field, CarEntityManager entityManager, CarGOManager goManager )
    {
      ownerNode_ = node;
      field_ = field;
      ...
      cnHierarchy_= CarManager.Instance.Hierarchy;

      id_ = FieldManager.CreateField( field.IsExclusive );
      SetScopeType( field_.ScopeType );
    }
    //-----------------------------------------------------------------------------------
    public void SetFieldContentType( CNFieldContentType allowedType )
    {
      if ( field_.ContentType != allowedType )
      {
        field_.ContentType = allowedType;
    ...
    {
      if (listSelectedIdx_.Count == 1)
      {
        List<uint> listNodeIdx, listWildcardIdx, listGameObjectIdx;
        GetSelectionLists(out listNodeIdx,out listWildcardIdx, out listGameObjectIdx);
        if (listWildcardIdx.Count == 1)
        {
          RenameSelection();
        }
      }
    ...
    public void BuildListItems()
    {
      ...
      if (nNameSelectors > 0)
      {
        listFieldItems_.Add( Tuple5.New("By GameObject Name:", "By GameObject Name:", ItemType.label, (Texture)null, -1) );
    ...
    {
      if ( field_.ContentType.IsFlagSet(CNFieldContentType.Locator) )
      {
        CarEditorUtils.GetSceneGameObjects(objects, listFilteredObjects);
      }
      if ( field_.ContentType.IsFlagSet(CNFieldContentType.Geometry) )
      {
        CarEditorUtils.GetGeometryGameObjects(objects, listFilteredObjects);
      }
      if ( field_.ContentType.IsFlagSet(CNFieldContentType.Animator) )
      {
        CarEditorUtils.GetSceneGameObjectsWithComponent<Animator>(objects, listFilteredObjects);
      }
    }
    //-----------------------------------------------------------------------------------
    ...
    public void AddNameSelectorWindow()
    {
      ...
      else if ( CarManagerEditor.IsOpen)
      {
        itemWindow = CNItemPopupWindow.ShowWindow( "Add name selector...", CarManagerEditor.Instance.position );
      }
      else
      {
        itemWindow = CNItemPopupWindow.ShowWindow( "Add name selector..." );
      }
    ...
    {
      Undo.RecordObject(ownerNode_, "CaronteFX - Convert references to game selectors");

      List<string> listNameSelector = new List<string>();
      List<GameObject> listGameObject = field_.GameObjects;
       
      int idx = listGameObject.Count - 1;
      for (int i = idx; i >= 0; i--)
      {
        GameObject go = listGameObject[i];
        string goName = go.name.ToLower();
        if ( !listNameSelector.Contains(goName) )
        {
          listNameSelector.Add(goName);
        }

        FieldManager.RemoveGameObject( (uint)id_, (uint)i );
      }

      listNameSelector.Reverse();

      foreach(string nameSelector in listNameSelector)
      {
        AddNameSelector(nameSelector, false);
      }

      cnHierarchy_.RecalculateFieldsDueToUserAction();

      Undo.SetCurrentGroupName("CaronteFX - CaronteFX - Convert references to game selectors");
      Undo.FlushUndoRecordObjects();
      Undo.CollapseUndoOperations(Undo.GetCurrentGroup());
    }
    #endregion
    ...
    public bool HasGameObjects()
    {
      ...
      return arrGameObjects.Length > 0;
    }
    //-----------------------------------------------------------------------------------
    public void EnableGameObjectSelection()
    {
      GameObject[] arrGameObjects = GetUnityGameObjects();
    ...
    //-----------------------------------------------------------------------------------
    public void DisableGameObjectSelection()
    {
      GameObject[] arrGameObjects = GetUnityGameObjects();
      foreach(GameObject go in arrGameObjects)
      {
    ...
    private List<GameObject> GetNodeDerivedObjects()
    {
      ...
      foreach( CommandNode cnode in listCommandNode )
      {
        ...
        {
          CommandNodeEditor nodeEditor = cnHierarchy_.GetNodeEditorSafe( (CommandNode)crTreeNode );
          if (nodeEditor != null)
          {
    ...
    {
      FieldManager.SetIsPureScope(id_, isScope);
    }
    //-----------------------------------------------------------------------------------
    ...
    public void FillOptionsMenu( GenericMenu optionsMenu )
    {
      if ( AreGameObjectsAllowed() )
      {
        ...
        optionsMenu.AddSeparator("");
        optionsMenu.AddItem(new GUIContent("Enable GameObjects"), false, EnableGameObjectSelection);
        optionsMenu.AddItem(new GUIContent("Disable GameObjects"), false, DisableGameObjectSelection);
        optionsMenu.AddSeparator("");
        optionsMenu.AddItem(new GUIContent("Select in Hierarchy"), false, SceneSelectionTopMost);
        optionsMenu.AddSeparator("");
        optionsMenu.AddItem(new GUIContent("Convert GameObject references to name selectors"), false, ConvertGameObjectReferencesToNameSelectors);
        optionsMenu.AddSeparator("");
      ...
      else
      {
        ...
        optionsMenu.AddSeparator("");
        optionsMenu.AddDisabledItem(new GUIContent("Enable GameObjects") );
        optionsMenu.AddDisabledItem(new GUIContent("Disable GameObjects") );
        optionsMenu.AddSeparator("");
        optionsMenu.AddDisabledItem(new GUIContent("Select in Hierarchy"));
        optionsMenu.AddSeparator("");
