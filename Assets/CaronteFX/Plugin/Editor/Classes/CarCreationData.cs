...
namespace CaronteFX
{
  public class CarCreationData
  {
    ...
    private bool smoothRenderMesh_;
    ...
    public CarCreationData(GameObject go, Mesh renderMesh, Mesh colliderMesh, CRBalltreeAsset btAsset, bool isRope, bool smoothRenderMesh)
    {
      gameObject_ = go;

      isRope_ = isRope;
      smoothRenderMesh_ = smoothRenderMesh;
    ...
    private bool AreMeshChecksValid(Caronte_Fx_Body fxBody)
    {
      ...
      else
      {
        ...
        {
          isBakedMesh = fxBody.GetColliderMesh(ref colliderMesh);
        }
...

      {
        ...
        bool isBakedMesh = fxBody.GetRenderMesh(ref renderMesh);
    ...
    private bool AreMeshFingerprintsValid(Caronte_Fx_Body fxBody)
    {
      ...
      {
        Mesh meshToCheck = null;
        bool isBakedMesh = false;
        if (isRope_)
        {
          meshToCheck = fxBody.GetTileMesh();
          if (fxBody.DoSmoothRopeRenderMesh != smoothRenderMesh_)
          {
            return false;
          }
        }      
        ...
        {
          isBakedMesh = fxBody.GetColliderMesh(ref meshToCheck);
        }
