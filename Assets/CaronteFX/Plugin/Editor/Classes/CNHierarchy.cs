...
namespace CaronteFX
{
  public class CNHierarchy : IListController
  {
    ...
    //-----------------------------------------------------------------------------------
    public CommandNodeEditor GetNodeEditorSafe(CommandNode node)
    {
      if (dictCommandNodeEditor_.ContainsKey(node))
      {
        return dictCommandNodeEditor_[node];
    ...
    {  
      ...
      if (nNodes > 0)
      {
        ...
        for (int i = 0; i < nNodes; i++)
        {
          ...
          {
            CommandNode parent = (CommandNode)(node.Parent);
            CommandNode clonedNode = CloneNode( parent, node, true );
        ...
        Undo.SetCurrentGroupName("CaronteFX - Duplicate selection");
        Undo.CollapseUndoOperations(Undo.GetCurrentGroup());
    ...
    //----------------------------------------------------------------------------------
    private CommandNode CloneNode(CommandNode parent, CommandNode node, bool firstCall)
    {
      CommandNode clone = (CommandNode)Undo.AddComponent( node.GetDataHolder(), node.GetType() );
      clone.Parent = null;
      clone.Children = new CRTreeNodeList(clone);

      clone.CloneData(node);
      ...
      {
        AttachChild(parent, clone, null);
      }

      CNGroup group = node as CNGroup;
      if (group != null)
      {
        int nChildren = group.ChildCount;
        for (int i = 0; i < nChildren; i++)
        {
          CloneNode( clone, (CommandNode)group.Children[i], false );
        }
      }
    ...
    public CNFracture CreateFractureNode( CNFracture.CHOP_MODE chopMode )
    {
      Action<CNFracture> postCreationAction = (CNFracture fracturerNode) =>
      {
        ...
        switch (chopMode)
        {
          case CNFracture.CHOP_MODE.VORONOI_UNIFORM:
            fracturerNode.Name = GetUniqueNodeName("FractureUniform");
            break;
          case CNFracture.CHOP_MODE.VORONOI_BY_GEOMETRY:
            fracturerNode.Name = GetUniqueNodeName("FractureByGeometry");
            break;
          case CNFracture.CHOP_MODE.VORONOI_RADIAL:
            fracturerNode.Name = GetUniqueNodeName("FractureRadial");
            break;
          default:
            break;
        }
      ...
      return ( CreateNodeUnique<CNFracture>("Fracture", postCreationAction) );
    }
    //----------------------------------------------------------------------------------
    ...
    public CNServos CreateServos(bool isLinearOrAngular, bool isPositionOrVelocity )
    {
      Action<CNServos> postCreationAction = (CNServos svNode) =>
      {
        ...
        {
          svNode.DistStepToDefineMultiplierDependingOnDist = 360.0f;
        }
      };
    ...
    private void AddDraggedObjects(CommandNode nodeTo, UnityEngine.Object[] arrGameObject)
    {
      ...
      {
        GenericMenu menu = new GenericMenu();
        menu.AddItem(new GUIContent("Create rigid bodies from selection"), false, () =>
        {
        ...
        menu.AddItem(new GUIContent("Create irresponsive bodies from selection"), false, () =>
        {
        ...
        menu.AddItem(new GUIContent("Create animated bodies from selection"), false, () =>
        {
        ...
        menu.AddItem(new GUIContent("Create soft bodies from selection"), false, () =>
        {
        ...
        menu.AddItem(new GUIContent("Create cloth bodies from selection"), false, () =>
        {
