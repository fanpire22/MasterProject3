...
namespace CaronteFX
{
  ...
  public class CNExplosionEditor : CNEntityEditor
  {
    ...
    private void DrawExplosionTransform()
    {
      ...
      if (Data.Explosion_Transform != null)
      {
        if (!eManager.IsBody(Data.Explosion_Transform.gameObject))
        {
          EditorGUILayout.HelpBox("Explosions can also be attached to a moving body.", MessageType.Info);
        }
      }
    }
