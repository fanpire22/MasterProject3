...
namespace CaronteFX
{
  public class CNAnimatedbodyEditor : CNRigidbodyEditor
  {
    ...
    static readonly GUIContent anAbsoluteTimeModeCt_                  = new GUIContent( CarStringManager.GetString("AnAbsoluteTimeSamplingMode"),                  CarStringManager.GetString("AnAbsoluteTimeSamplingModeTooltip" ) );
    ...
    private void SampleCaronteFX(GameObject go, out bool isVertexAnimated)
    {
      ...
      if (crAnimation != null)
      {
        ...
        crAnimation.LoadActiveAnimation(true);
        crAnimation.SetFrame(0.0f);

        isVertexAnimated = crAnimation.HasVertexAnimatedObjects(go);

        SkinnedMeshRenderer smr = go.GetComponent<SkinnedMeshRenderer>();
        if (smr != null)
        {
          isVertexAnimated = true;
          SetBonesLocalScaleToOneIfApproximatelyZero(smr);
        }
    ...
    //----------------------------------------------------------------------------------
    private void SetBonesLocalScaleToOneIfApproximatelyZero(SkinnedMeshRenderer smr)
    {
      ...
      if (arrBone != null)
      {
        foreach(Transform bone in arrBone)
        {
          if (bone != null)
          {
            bool scaleWasZero = CarEditorUtils.SetLocalScaleToOneIfApproximatelyZero(bone);
      ...
      if (rootBone != null)
      {
        bool scaleWasZero = CarEditorUtils.SetLocalScaleToOneIfApproximatelyZero(rootBone);
    ...
    private void RestoreCaronteFX()
    {
      for (int i = 0; i < listCRAnimation_.Count; i++)
      {
        ...
        crAnimation.CloseActiveAnimation();
    ...
    //----------------------------------------------------------------------------------
    public void Sample(GameObject go)
    {
      if (Data.AnimationType == CNAnimatedbody.EAnimationType.Animator)
      {
        SampleAnimationController(go);
    ...
    {
      eManager.CheckBodyForChanges(this, Data, go, recreateIfInvalid);
    }
    //----------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------
    private void DrawAbsoluteTimeAnimationSamplingMode()
    {
      EditorGUI.BeginChangeCheck();
      var value = EditorGUILayout.Toggle(anAbsoluteTimeModeCt_, Data.AbsoluteTimeSamplingMode);
      if (EditorGUI.EndChangeCheck())
      {
        Undo.RecordObject(Data, "Change " + anAbsoluteTimeModeCt_.text);
        Data.AbsoluteTimeSamplingMode = value;
        EditorUtility.SetDirty(Data);
      }
    }
    //----------------------------------------------------------------------------------
    protected override void RenderFieldsBody(bool isEditable)
    {
      ...
      EditorGUILayout.Space();
      DrawAbsoluteTimeAnimationSamplingMode();
